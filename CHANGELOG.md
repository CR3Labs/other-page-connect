...
- Improve logic for `isConnecting` within the Custom `ConnectButton`.
...
- Included `chain` to `ConnectButton.custom` to allow for chains to be passed to custom buttons.
...
- Add props to `<ConnectButton />` for additional button configuration
