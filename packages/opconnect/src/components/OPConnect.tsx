...
import OPConnectModal from './ConnectModal';
...
export type OPConnectOptions = {
  ...
  avoidLayoutShift?: boolean; // Avoids layout shift when the OPConnect modal is open by adding padding to the body
...
export const OPConnectProvider = ({
  children,
  // theme = 'auto',
  mode = 'auto',
  primaryColor = '#F97316',
  ...
  onDisconnect,
  debugMode = false,
}: OPConnectProviderProps) => {
  // OPConnectProvider must be within a WagmiProvider
  if (!React.useContext(WagmiContext)) {
    throw Error('OPConnectProvider must be within a WagmiProvider');
  }

  // Only allow for mounting OPConnectProvider once, so we avoid weird global
  // state collisions.
  if (React.useContext(Context)) {
    throw new Error(
      'Multiple, nested usages of OPConnectProvider detected. Please use only one.'
    );
  }
  ...
  const defaultOptions: OPConnectOptions = {
    language: 'en-US',
    hideBalance: false,
    hideTooltips: false,
  ...
  const opts: OPConnectOptions = Object.assign({}, defaultOptions, options);
...
export const useContext = () => {
  ...
  if (!context) throw Error('OPConnect Hook must be inside a Provider.');
  return context;
};
